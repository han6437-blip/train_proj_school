# 企业机房与办公电脑维修 Agent 系统：面试回答总册

本目录包含上一轮题库中全部问题的参考回答。回答统一采用“先给结论—解释实现—说明异常与权衡—回到项目结果”的结构，适合整理成 3～5 分钟口述。

如果你是初学者，请先阅读[项目基础课：从后端、幂等与 Outbox/Inbox 到 Agent、RAG 和微调](./beginner_tutorial.md)，再进入题库。当前目录快照未包含文中提到的教学示例目录，因此应把相关代码视为讲解片段，不能表述为本地已经运行验证。

## 使用前必须确认的真实信息

以下内容来自已经给出的项目材料，可以在回答中保持一致：

- 业务场景：企业机房、服务器与办公电脑故障咨询，资产和机房信息收集，维修工程师匹配、上门预约和用户偏好沉淀。
- Agent 与编排：LangGraph `StateGraph` 的在线图包含 Router、Supervisor 决策节点，以及咨询、预约两个专业 Agent 节点/子图；用户行为分析 Agent 异步消费已完成事件，不由 Supervisor 在线编排。对外可概括为 3 类专业能力，但必须说明运行链路不同。
- 工程规模：Web/API/Agents/Services/DB 五层、16 个 API、30 个场景测试。
- RAG：BM25 + Dense 双路召回、RRF 融合、Cross-Encoder 精排，精排失败回退 RRF。
- 记忆读路径：最近 10 轮、窗口使用率 60% 触发摘要、召回 Top-5；这些都是首版可调参数。
- 记忆写路径：Event → 原子 Candidate → Policy → 稳定 Key → 唯一 Operation → 幂等事务/CAS → 派生索引；长期保存带来源和版本的低风险偏好与必要事件摘要。
...
1. **编排方式**：外层使用 LangGraph `StateGraph`。确定性 Router 让简单单意图直接进入咨询或预约节点；复合、多步骤任务进入 Supervisor 决策节点，由条件边或 `Command(goto=...)`路由到专业 Agent 节点/子图。专业 Agent 内部可以调用检索、排班和预约等业务工具，但 Agent 本身不包装成 Supervisor 的 Tool；最终副作用由业务服务执行，而不是由模型直接操作数据库。用户行为分析通过异步事件链路运行。
...
9. **记忆写入口径**：同步明确记住/更正/删除与异步普通提取共用一套事件和操作协议；关系型状态与历史版本是权威记忆源，全文/向量/图仅为可重建的检索视图，长期记忆不能覆盖预约、排班和权限事实。

## 文档索引

- [项目完整请求流程与代码映射](./project_request_flow.md)
- [企业机房与办公电脑维修 Agent：记忆系统工程化设计](./memory_system_design.md)
...
- [Langfuse + OpenTelemetry：Agent Trace 方案与面试重点](./langfuse_opentelemetry_tracing_interview_tutorial.md)
...
- [Langfuse：OpenTelemetry 原生集成](https://langfuse.com/integrations/native/opentelemetry)
