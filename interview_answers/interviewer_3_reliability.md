...
我的做法是按负载隔离：原生异步 I/O 直接 `await`；不可异步化的短阻塞调用放进有界线程池；CPU/GPU 密集任务放到独立进程或模型服务，并在入口设置并发信号量、有界队列和绝对 Deadline。Deadline 还要向下游传播；调用方超时只表示“不再等待”，不能假定线程函数、GPU Kernel 或远端任务已经停止，因此不可抢占的任务仍要隔离、计入容量并在后台回收或对账。`asyncio.to_thread()`只能防止主事件循环被阻塞，不能增加 CPU 或显存容量。验证时我会观察事件循环延迟、队列等待、CPU/显存、吞吐、P95/P99 和超时率，而不是只看代码里有没有 `async`。
...
我守住的不变量是：同一租户、同一工程师的两个有效预约区间不能重叠。SQLite 中，在所有预约业务写路径都只能经过 Booking Service 的信任边界内，可以用 `BEGIN IMMEDIATE`串行化短写事务并在锁内复查；但这不是声明式的区间约束，若允许旁路直写就可能失效，所以要通过数据库文件访问权限和进程边界保证唯一写入口。PostgreSQL 中我更倾向使用 `btree_gist`、range type 和 GiST 排斥约束：参与不变量的租户、工程师、起止时间和占用状态都设为 `NOT NULL`，用 `tstzrange(start_at, end_at, '[)')`表示半开区间，增加 `CHECK (start_at < end_at)`，再对 `tenant_id`、`engineer_id`做 `=`约束，对 `during`做 `&&`约束，并用部分约束只覆盖有效状态。这样前一单结束时可以紧接下一单；即使两个事务都曾读到空闲，数据库仍会拒绝冲突插入或状态更新。应用捕获约束冲突后回滚，返回冲突并重新给候选，而不是原样重试过期方案。
...
幂等记录至少要覆盖客户端重试、工作流恢复和对账窗口。完整响应到期后可以压缩成轻量墓碑，但墓碑仍要保存可信作用域、Key、规范化参数哈希以及业务结果或 `operation_id`引用，并覆盖系统承诺的最大迟到重放窗口；只有仍保留墓碑时，服务端才能识别并拒绝旧 Key 的异参数复用。若把记录彻底删除，就必须明确声明 TTL 之后不再保证该 Key 的幂等性，不能声称还能识别旧 Key。若用户确实要创建参数相同的新预约，需要新的业务意图和新 Key。关键验证包括：并发同 Key、同 Key 异参数、首次提交成功但响应丢失、完整结果压缩为墓碑后的重放，以及彻底过期后的契约行为。
...
- 死锁、序列化失败、SQLite BUSY：先确认失败事务已经回滚或根本未开始，再开启新事务，重跑完整业务单元并加 Jitter；Session 正确 `rollback`后通常可以复用，只有连接失效或状态不可恢复时才重建 Session/连接。
...
我常用 Full Jitter：第 `n` 次从 `0` 到 `min(cap, base × 2^n)`之间随机等待；若下游给出 `Retry-After`，实际重试不能早于它指定的时间，若等待后会超过端到端 Deadline 就直接失败。重试还要限制最大尝试次数，并设置全局预算和并发上限；熔断打开后停止常规请求，只允许少量探测。Jitter 只能缓解恢复流量，不能把权限错误变成可重试错误，也不能解决副作用结果未知。
...
核心原则是：数据库提交前绝不流式宣告“预约成功”。解释文本可以提前发，但业务结果只在事务成功提交后，通过结构化 `final_result`事件发送。每个事件携带 `operation_id`、事件类型和递增序号，例如 `content_delta`、`tool_status`、`final_result`、`error`；在 SSE 协议层还要把 `(operation_id, seq)`编码进 `id:`字段，这样标准 `EventSource`重连时才会自动发送 `Last-Event-ID`。服务端校验该游标属于当前用户和流后，从有保留期限的持久事件缓冲继续重放；非 `EventSource`客户端则要显式回传 cursor。若缓冲已过期，就直接按 `operation_id`查询数据库终态。

HTTP 响应头发出后不能再改成 500；连接仍在就发送结构化流错误事件，连接已断就结束。流错误只说明传输或本轮处理异常，不自动等于预约失败：客户端既没收到 `final_result`，也没收到业务服务明确给出的“确认无副作用”的 `terminal_failure`时，才把结果判断为 `UNKNOWN`并按 `operation_id`查询终态。若事务已提交而最终事件丢失，重连、状态查询或幂等重放仍返回同一个业务结果。高风险结论使用数据库事实生成的模板事件，避免模型提前自由表述成功状态。

## 11. 分布式租约过期，但旧任务仍在运行怎么办？

租约过期后，新 Worker 可能取得锁，而暂停过的旧 Worker 又恢复写入，这叫 stale holder。只有租约不足以阻止旧持有者，所以我会增加 fencing token。

获取新租约时要在权威租约存储中按业务资源原子递增 `run_epoch`，每次写偏好、聚合结果或 Checkpoint 都携带该 token。若租约表与业务数据在同一数据库，条件更新要在同一事务中校验 epoch 仍是当前值，必要时同时校验 owner 和 `expires_at`；若目标存储只能维护 `last_fence`，则由更高 token 原子推进 fence，并拒绝小于 `last_fence`的写。判断必须在存储层完成，不能让客户端基于旧快照比较。

这里的保证边界要说清：仅仅“租约已过期”不会自动让所有存储拒绝旧任务。在 `last_fence`方案中，只有新持有者取得更高 epoch 并让它在目标存储生效后，旧 token 的后续写才一定被拒绝；在此之前，恢复的旧任务仍可能写入。同一个 token 的重复写还要靠稳定 `event_id`、业务版本或唯一约束去重。续租失败后 Worker 立即停止取新批次，业务更新与对应 Checkpoint 尽量同事务提交；外部系统若不支持 fencing，仍要使用幂等键、状态查询和对账处理未知结果。
...
消费者先用`event_id`去重，再按白名单 Schema 生成原子 Candidate，并以`tenant + user + preference_field + applicability`形成稳定 Key。聚合视图带 `version`，Memory Operation 带唯一`operation_id`和`expected_version`，更新使用 `WHERE version = old_version`做乐观锁；CAS 失败就重读最新状态并重新比较，不能让最后提交者静默覆盖。

计数累加通常可交换，但“最新显式纠正覆盖旧偏好”需要定义来源优先级、服务端顺序和适用范围。“本周六可以”是当前任务例外，不应覆盖“默认避开周末”；只有“以后周末也可以”才更新长期默认偏好。短期会话约束与长期偏好必须分开，偏好也永远不能覆盖排班和预约的数据库事实。

## 13. llama.cpp 的 `--parallel`、`--ctx-size` 与并发是什么关系？

`--parallel`控制 server slot 上限，也就是最多可并行承载的序列数；`--ctx-size`在当前 server 构建中通常表示总上下文或 KV Token 预算，但具体语义要以固定版本为准。非 unified KV 配置下，每个 slot 的有效上下文大致受 `ctx-size / parallel`约束；启用 unified KV 后，多个序列动态共享总池，空闲时单个请求可能使用超过平均份额，因此不能再把 `ctx-size / parallel`当成固定的单请求上限，但并发最坏情况下每个请求可保证的预算仍会下降。增加 slot 可以配合 continuous batching 提高总吞吐，却不会凭空增加总 KV 容量，还会加剧算力竞争，TTFT 和尾延迟可能变差。
...
高风险确认要绑定动作名、规范化参数摘要、资源版本、用户与租户、有效期和一次性语义。一次性不能只靠字段或签名声明：执行时要对服务端确认记录做 CAS，例如只在 `used_at IS NULL`、未过期且参数摘要一致时原子写入 `used_by_operation_id`；受影响行数必须为 1，并尽量让确认消费、幂等记录和 Booking 写入处于同一事务。执行前再检查资源归属、权限、限额和最新数据库事实。即使 Prompt Injection 生成了格式完全正确的 Tool Call，也不代表它获得了执行权。测试时我会做跨用户恢复、篡改确认参数、重放过期确认、两个请求并发消费同一确认、并发批准/拒绝和 IDOR 负样本。

## 18. 如何防止跨用户、跨租户数据泄露？

我不会信任用户可控的 `thread_id`、请求头里的 `tenant_id`或模型生成的身份参数。服务端从已验证 Token 获取 actor、tenant 和 role，把它们放入内部操作与线程命名空间。仓储层强制按 `tenant_id + resource_id`查询并复核对象归属；高隔离场景再用 PostgreSQL RLS、独立 schema 或数据库做纵深防御。使用 RLS 时，应用账号必须是 `NOBYPASSRLS`且不能是表 owner，必要时对表启用 `FORCE ROW LEVEL SECURITY`；连接池中的租户上下文用事务级 `SET LOCAL`注入，让事务结束时自动清理，避免把上一个请求的租户状态带给下一个请求。
...
我先定义安全不变量，再定义可用性目标。前者包括：有效预约不重叠；未绑定有效确认时，不得执行需要确认的创建、修改或取消预约命令；同 Key 同语义最多一个 Booking；同 Key 异参数必须冲突；Booking 与 Outbox 同事务；跨租户数据不返回；通知失败不能回滚已提交预约。后者再用成功率、P95/P99、恢复时间、队列年龄和积压衡量，不能为了成功率放松权限或一致性。
